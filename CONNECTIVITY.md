...
peer semantic-message bodies use Bencodex, while the hub iframe WebSocket
remains JSON.
...
messages and all later protocol messages as session-scoped reliable frames
inside Bencodex `relay` dictionaries. The first reliable data message is
`session_proposal`; its frame carries a random 16-byte peer session ID generated
by the initiator. Every data, acknowledgement, and keepalive frame carries that
...
instead of independent routing and reliability refs. The transport owns the
peer ID, session ID, liveness, ordered delivery, acknowledgement state, replay
log, and outbound framing from consent negotiation onward. Accepting a proposal
attaches the WASM consumer without resetting transport counters. Destroying the
...
- **Own hub player ID changes on registration** → the old peer route is
  ...
  already submitted or resolution is already on-chain.
...
4. If the target consents, it marks itself busy, generates a random 16-byte
   peer session ID, persists a reliable transport epoch, and sends
   `session_proposal` as data message 1 under that ID. It then starts WASM as
   initiator and sends handshake messages through the same counter sequence.
   Persist of that start is async; `session_reject` / local cancel must abort
   any in-flight start so it cannot resurrect an orphan handshake.
5. The challenger app checks local availability before showing the proposal
   ...
   point; the challenger durably records an admitted proposal before
   acknowledging it and rejects a proposal whose `network` is missing,
   ...
   the prompt is open the same transport can order early `HandshakeA` bytes
   behind the proposal. If unavailable or declined, it sends `session_reject`;
   the application attempt is cancelled while the terminal rejection remains
   replayable until acknowledged or bounded retention expires. The receiver
   persists a minimal receipt before acknowledging, allowing a lost
   acknowledgement to be re-sent after reload without repeating cancellation.
   Receiving
...
- **Peer relay**: Addressed Bencodex `relay` dictionaries through the hub
  WebSocket pipe. `PeerSession` and its reliability owner hold the session ID,
  numbered ACK protocol, reorder queue, replay log, and keepalive
  (`front-end/src/services/PeerSession.ts`). `SessionController` attaches the
...
cancelled/error, and do not leave an orphan handshake. After channel
establishment, the same message is a peer protocol failure and triggers the
existing on-chain path rather than cancelling the obligation. When the channel
