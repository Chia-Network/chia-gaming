...
from clvm_types.serialized_program import SerializedProgram
from clvm_types.lock import Lockfile
...
def compile_clvm_in_lock(full_path: pathlib.Path, output: pathlib.Path, search_paths: List[pathlib.Path]):
    ...
    print("X", str(full_path), str(output), treated_include_paths)
...
def load_serialized_clvm(
    ...
    z = str(clvm_filename).split('.')
    hex_filename = f"{clvm_filename}.hex"
    #print(f"AAA: {hex_filename}")
    if z[-1] == "clsp":
        hex_filename = f"{'.'.join(z[:-1])}.hex"
        #print(f"BBB: {hex_filename}")
    ...
    if recompile:
        try:
            if True: #pkg_resources.resource_exists(package_or_requirement, clvm_filename):
                # Establish whether the size is zero on entry
                full_path = pathlib.Path.cwd() / clvm_filename
                    #pkg_resources.resource_filename(package_or_requirement, clvm_filename))
                                         
                output = full_path.parent / hex_filename
                #output = full_path / hex_filename
    ...
    with open(hex_filename, mode="r", encoding="utf-8") as f:
        clvm_hex = f.read()
        assert len(clvm_hex.strip()) != 0
        clvm_blob = bytes.fromhex(clvm_hex)
        p = SerializedProgram.from_bytes(clvm_blob)
        # print (f"ZZZ: {p.get_tree_hash().to_hex()}")
        return p
