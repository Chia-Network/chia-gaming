...
pull_out_cards_tests = [
    {"selections": 0b00011111,
     ...
     "expected": [[b'\x02', b'\x01'], [b'\x02', b'\x02'], [b'\x02', b'\x03'], [b'\x02', b'\x04'], [b'\x03', b'\x01']],
...
split_cards_tests = [
    {"selections": 0b00001111,
     "cards": [bytes([x]) for x in range(8)],
     "expected": [[b'\x07', b'\x06', b'\x05', b'\x04'], [b'\x03', b'\x02', b'\x01', b'\x00']],
     "msg": "split_cards xxx"}]
def test_split_cards(args):
    ...
    dbg_assert_eq(args["expected"], ret.as_python(), f"test_split_cards({args}): {args['msg']}")
...
def test_get_final_cards_in_canonical_order(args):
    ret = test_handcalc_micro.run(["get_final_cards_in_canonical_order",
                                   args["alice_cards"], args["alice_picks"], args["bob_cards"], args["bob_picks"]])
    dbg_assert_eq(args["expected"], ret.as_python(), f"test_get_final_cards_in_canonical_order({args}): {args['msg']}")
