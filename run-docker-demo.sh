...
docker run --platform linux/amd64 -i -p 127.0.0.1:3000:3000 -p 127.0.0.1:3001:3001 -p 127.0.0.1:5800:5800 "${@}" -t chia-gaming-wim 2>&1 | grep -v 'updating the mempool using the slow-path'
