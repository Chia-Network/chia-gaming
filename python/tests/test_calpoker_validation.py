...
def run_game(
    ...
    last_move: Move,
    max_move_size_from_last_turn: int,
    script,
    indent=0,
    path="",
):
    if isinstance(script, TestCaseSequence):
        ...
        print(
            f"comparing validator name {validator_info.name} to expected {script[-1]}"
        )
...
def generate_test_set(test_inputs: Dict):
    ...
    c_move_bad_alice_reveal_move = (
        alice_bad_seed + sha256(alice_discards_salt + alice_discards_byte).digest()
    )
    ...
    e_move_too_few_discards = (
        alice_discards_salt + too_few_discards + alice_good_selections
    )
    e_move_too_many_discards = (
        alice_discards_salt + too_many_discards + alice_good_selections
    )
    e_move_too_few_selections = (
        alice_discards_salt + alice_discards_byte + too_few_selections
    )
    e_move_too_many_selections = (
        alice_discards_salt + alice_discards_byte + too_many_selections
    )
    ...
    recursive_list_up_to_d = [
        ...
        [
            ...
            (
                alice_discards_salt + alice_discards_byte + alice_loss_selections,
                0,
                bob_good_selections,
                MoveCode.SLASH,
                False,
    ...
    return TestCaseAlternative([wrong_alice_picks_count_tests, slash_succeed_tests])


def test_run_with_moves(move_list, amount):
    step_a = load_clvm_hex(calpoker_onchain_clsp_dir / "a.hex")
    ...
    move_zero = Move(
        MoveCode.MAKE_MOVE,
        next_validator_hash=step_a_hash,
        state=Program.to(0),
        next_max_move_size=len(step_a_hash),
        extra_data=Program.to(0),
    )
...
def normal_outcome_move_list():
    ...
    alice_discards_byte = 0b01010101.to_bytes(
        1, byteorder="big"
    ...
    entropy_values = [
        bytes.fromhex(
            "eb04c21e3ee58d1b494e0b5be68ee5e5ae5d4b7a0a01287005ff21e7b70c5ddc"
        ),
        bytes.fromhex(
            "ce173df1d1a7f2854f87d48cee0b17ac59dfad7b3d7ca077009b84808ae25b20"
        ),
        bytes.fromhex(
            "2b0433e13d49320ef10de4025b793b33df30ead99660f49b4dd4d11c836a407e"
        ),
        bytes.fromhex(
            "55218743c4fd53281f871d079483ace7cbf92d0e269093c23febd9f5e1b0dd44"
        ),
        bytes.fromhex(
            "5dec7b7c6c954f9f256900d7f67f2ab0b51f98ae7ee7bd71831eab4d62193b54"
        ),
    ]
    ...
    return [
        ...
        (
            alice_discards_salt + alice_discards_byte + bob_selects_byte,
            0,
            None,
            MoveCode.MAKE_MOVE,
            False,
