...
describe('WASM wallet funding requests', () => {
  ...
  it('retains an insufficient-balance funding request for manual retry', async () => {
    ...
    const createOfferForIds = jest
      ...
      .mockResolvedValueOnce({ offer: 'offer1funded', tradeId: 'trade-funded' });
    const blockchain = new BlockchainPoller({ ...mockRpc, createOfferForIds }, 60000);
    const { blob, cradle } = createReadyBlob();
    ...
    blob.getObservable().subscribe((event) => {
      if (event.type === 'funding-retry') retryErrors.push(event.error);
    });
    setActiveBlob(blob);
    blob.blockchain = blockchain;
    ...
    expect(retryErrors).toEqual([expect.stringMatching(/retry funding/i)]);
    expect(walletCallbackFailed).not.toHaveBeenCalled();
    expect(blob.retryFundingOffer()).toBe(true);
    await blob.flushPendingWork();

    expect(createOfferForIds).toHaveBeenCalledTimes(2);
    expect(cradle.provide_offer_bech32).toHaveBeenCalledWith('offer1funded');
