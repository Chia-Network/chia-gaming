...
The Bencodex primitive encoding used by control dictionaries is defined in
[`PEER_PROTOCOL.md`](PEER_PROTOCOL.md#5-bencodex-encoding). Control dictionaries
use text keys and a required text field named `type`.
...
## 4. Connection and identity lifecycle
...
## 5. Player-to-hub control messages
...
## 6. Hub-to-player control messages
...
This message is advisory and untrusted. It does not start the Rust peer
protocol. The player validates it, checks local availability, and asks for
local consent. If unavailable, the player ignores it.

Hub policy determines when this advisory is emitted. Amount names are from the
recipient's perspective.

### 6.3 `delivery_failure`
...
[`PEER_PROTOCOL.md`](PEER_PROTOCOL.md#4-reliable-peer-frames).
...
peer receive policy separately defaults to a 10 MiB authoritative Rust message
body limit.
