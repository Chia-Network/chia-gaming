...
import {doInternalLoadWasm} from '../hooks/WasmStateInit';
...
*/
  ...
  loadPresets(presetFiles: string[]) {
    ...
    return Promise.all(presetFetches).then((presets) => {
      ...
      const rngId = this.wasmConnection?.create_rng(this.rngSeed);
      if (rngId === undefined) {
        throw("Unvalid rngId");
      }
      this.rngId = rngId;
      const newGameIdentity = this.wasmConnection?.chia_identity(rngId);
  ...
  handleOneMessage(msg: any): any {
    ...
    if (msg.loadWasmEvent) {
      ...
      return this.internalTakeBlock(
        ...
        msg.takeBlockData.block_report,
      );
    }
  ...
  takeOpponentMove(
    moveNumber: number,
    game_id: string,
    readable_move_hex: string,
  ): any {
    const result: any = {
      setMyTurn: true,
    };
    console.log('takeOpponentMove', moveNumber, game_id, readable_move_hex);
    const p = decode_sexp_hex(readable_move_hex);
  ...
  createStartCoin(): any {
    ...
    console.log(
      `create coin spendable by ${identity.puzzle_hash} for ${this.perGameAmount}`,
    );
    return this.relayAddress()
      ...
      .then((result) => {
        ...
        if (typeof coin !== 'string') {
          ...
          const new_coin_string = this.wasmConnection?.convert_coinset_to_coin_string(
  ...
  generateEntropy() {
    ...
    const entropy = this.wasmConnection?.sha256bytes(hexDigits.join(''));
    if (!entropy) {
  ...
  blockNotification(peak: number, blocks: any[], block_report: any) {
    if (block_report === undefined) {
      ...
      for (const block of blocks) {
        const one_report =
          this.wasmConnection?.convert_coinset_org_block_spend_to_watch_report(
