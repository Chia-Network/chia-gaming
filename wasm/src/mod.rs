mod gaming_wasm {
    ...
    struct JsGameSession {
        ...
        cradle: TransactionManager<GameSession>,
    }
    ...
    const GAME_SESSION_SERIALIZATION_SCHEMA: u32 = 12;
    ...
    fn watch_coin_entries(cradle: &JsGameSession) -> Vec<JsWatchCoinEntry> {
    ...
    /// Advance protocol clocks from a confirmed peak without claiming that an
    ...
    #[wasm_bindgen]
    pub fn start_handshake(cid: i32, opening_fee: &str) -> Result<JsValue, JsValue> {
        let fee = opening_fee
            .parse::<u64>()
            .map(Amount::new)
    ...
    pub fn propose_games(cid: i32, games: JsValue) -> Result<JsValue, JsValue> {
        let js_games: Vec<JsGameProposal> = serde_wasm_bindgen::from_value(games).into_js()?;
        with_game(cid, move |cradle: &mut JsGameSession| {
            ...
            let ids = cradle
                .cradle
                .propose_games(&mut cradle.allocator, &game_starts)?;
