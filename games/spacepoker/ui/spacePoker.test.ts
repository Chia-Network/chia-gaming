...
describe('Space Poker feature-state authority', () => {
  ...
  it('commits a fold and its terminal presentation as one codec-valid state', () => {
    ...
    const controller = {
      ...
      transitionFeatureState: (_gameType: string, _gameId: string, state: unknown) => {
        ...
        return spacepokerRegistration.decodeFeatureState( state) !== null;
      },
    ...
    expect(spacepokerRegistration.decodeFeatureState( transitions[1])).toMatchObject({
      gameState: { handler: SpHandler.MidRound, myTurn: true, N: 3n },
      terminalState: 'none',
      handHistory: [],
      pendingTerminalAction: null,
  ...
  it.each([
    ...
    expect(spacepokerRegistration.decodeFeatureState( committed[0].state)).toMatchObject(
      action === 'raise'
