...
export function reduceSpacepokerDurableState(
  ...
  if (event.type === 'accepted-group') {
    ...
    return current ?? initialState(event.isMyTurn, spacepokerTermsOf(event.terms)?.unitSizeMojos ?? 1n);
  }
...
export function validateSpacepokerTerms(terms: HandTermsModel): boolean {
...
export const spacepokerRegistration: GameFeatureRegistration<
  SpacepokerHandState,
  SpacepokerHandState,
...
> = {
  ...
  describeTerms(terms, { formatMojos }) {
    const space = spacepokerTermsOf(terms);
    if (!space) return `Stake ${formatMojos(terms.myContribution)} each`;
  ...
  compose: {
    ...
    draftFromTerms: (terms) => {
      const unitSize = spacepokerTermsOf(terms)?.unitSizeMojos ?? 1n;
      return {
        unitSize,
        stackSize: unitSize > 0n ? terms.myContribution / unitSize : 10n,
      };
    },
  ...
  toFactoryParameters(terms, iStarted) {
    const betUnit = resolveSpacepokerUnitSize({ terms });
    if (!betUnit || !this.validateTerms(terms)) {
  ...
  persistence: {
    ...
    decodeExtras(base, extras) {
      ...
      try {
        ...
        const terms = { gameType: 'spacepoker', ...base, unitSizeMojos };
        return validateSpacepokerTerms(terms) ? terms : null;
      } catch {
        return null;
      }
