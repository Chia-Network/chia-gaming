...
pub fn prefix_test_moves(allocator: &mut AllocEncoder, game_id: GameID) -> Vec<SimScriptAction> {
    ...
    assert_eq!(
        selected_cards_to_bitfield(&alice_hand, &alice_discards),
        0b0101_0101
    );
    assert_eq!(
        selected_cards_to_bitfield(&bob_hand, &bob_discards),
        0b1010_1010
    );
...
mod sim_tests {
    ...
    fn extract_info_from_messages(
        ...
        game_results
            .iter()
            .find_map(|x| {
                if let SimScriptActionResult::MoveResult(_, _, Some(clvm_data), _) = x {
    ...
    fn game_run_outcome_to_move_results(g: &GameRunOutcome) -> Vec<SimScriptActionResult> {
        let mut output: Vec<SimScriptActionResult> = Vec::new();
            ...
            iters[who].next()
        {
            ...
            output.push(SimScriptActionResult::MoveResult(
                readable_move.clone(),
                Vec::new(),
                message,
                Hash::default(),
    ...
    pub fn test_funs() -> Vec<(&'static str, &'static (dyn Fn() + Send + Sync))> {
        ...
        res.push(("test_opening_parity_with_main_vectors", &|| {
            ...
            match &moves[2] {
                SimScriptAction::Move(player, _, readable_move, _) => {
        ...
        res.push(("test_verify_endgame_data", &|| {
            ...
            if let SimScriptActionResult::MoveResult(readable_data, _, _, _) = last_result {
                ...
                panic!(
                    "expected MoveResult for final game action, got: {:?}",
                    last_result
                );
            }
        ...
        res.push(("test_play_calpoker_end_game_reward", &|| {
            ...
            assert_calpoker_moves_have_data(
                &game_outcome.local_uis[0].events,
                "end_game_reward p0",
            );
            assert_calpoker_moves_have_data(
        ...
        res.push(("test_two_games_initiator_proposes", &|| {
            ...
            moves.push(SimScriptAction::AcceptProposal(1, LocalProposalId(1)));
            moves.extend(prefix_test_moves(&mut allocator, GameID(1)));
            ...
            moves.push(SimScriptAction::AcceptProposal(1, LocalProposalId(3)));
        ...
        res.push(("test_two_games_joiner_proposes", &|| {
            ...
            moves.push(SimScriptAction::AcceptProposal(0, LocalProposalId(0)));
            moves.extend(
            ...
            moves.push(SimScriptAction::AcceptProposal(0, LocalProposalId(2)));
            moves.push(SimScriptAction::WaitBlocks(11, 0));
