...
fn load_validators(allocator: &mut AllocEncoder) -> ValidatorLibrary {
    ...
    for name in &VALIDATOR_NAMES {
        let path = format!("games/calpoker/clsp/onchain/{name}.hex");
...
fn run_referee_slash_case(
    ...
    let a_validator = read_hex_puzzle(allocator, "games/calpoker/clsp/onchain/a.hex")
...
fn expected_infohash_b_for_valid_a_move(
    ...
    let b_validator = read_hex_puzzle(allocator, "games/calpoker/clsp/onchain/b.hex")
...
pub fn test_funs() -> Vec<(&'static str, &'static (dyn Fn() + Send + Sync))> {
    vec![
        ...
        (
            "test_terminal_coin_nil_infohash_b_slash_invalid_move",
            &test_terminal_coin_nil_infohash_b_slash_invalid_move,
        ),
    ]
}
...
fn test_terminal_coin_nil_infohash_b_slash_invalid_move() {
    ...
    // Run step e with mover_share=0 to confirm it's a slash (Alice cheated)
    let e_result = run_validator_step(
        &mut a,
        &lib,
        &e_validator_hash,
    ...
    .expect("step e validator should run");
    assert_eq!(e_result.move_code, MoveCode::Slash);
    ...
    assert!(
        result.is_ok(),
        "slash must succeed on terminal coin when the move was invalid (mover cheated)"
    );
}
