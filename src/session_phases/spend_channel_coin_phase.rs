...
impl PeerLifecyclePhase for SpendChannelCoinPhase {
    ...
    fn timeout_claim_submitted(
        ...
        if matches!(semantic, TimeoutClaimSemantic::ChannelTimeoutFinish) {
            SpendChannelCoinPhase::timeout_claim_submitted(self, semantic);
        }
        Ok(None)
    }
    fn timeout_claim_rearmed(
        ...
        if matches!(semantic, TimeoutClaimSemantic::ChannelTimeoutFinish) {
            SpendChannelCoinPhase::timeout_claim_rearmed(self, semantic);
        }
        Ok(None)
    }
    #[cfg(test)]
...
mod tests {
    ...
    fn timeout_submission_reorg_transition_returns_snapshot_to_waiting() {
        ...
        assert!(
            <SpendChannelCoinPhase as PeerLifecyclePhase>::timeout_claim_submitted(
                &mut phase,
                TimeoutClaimSemantic::ChannelTimeoutFinish,
            )
            .expect("timeout submission")
            .is_none()
        );
        assert_eq!(
            phase.channel_status_snapshot().unwrap().semantic_phase,
        ...
        assert!(
            <SpendChannelCoinPhase as PeerLifecyclePhase>::timeout_claim_rearmed(
                &mut phase,
                TimeoutClaimSemantic::ChannelTimeoutFinish,
            )
            .expect("timeout rearm")
            .is_none()
        );
        assert_eq!(
            phase.channel_status_snapshot().unwrap().semantic_phase,
