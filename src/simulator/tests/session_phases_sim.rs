...
use crate::session_phases::effects::{
    CancelReason, ChannelStatus, ChannelStatusSnapshot, FailedGameAction, GameNotification,
    GameSessionEvent, GameStatusKind, LocalActionKind, SettlementOutcome, UnrollInitiator,
};
...
use crate::session_phases::types::{
    BatchAction, ChannelFundingWallet, GameAction, PacketSender, PeerMessage, ToLocalUI,
    WalletSpendInterface, WireProposal,
};
        ...
        (TestEvent::Notification(actual_n), ExpectedEvent::Notification(expected_n)) => {
            match (actual_n, expected_n) {
                ...
                (
                    ...
                    ExpectedNotification::ProposalAcceptedGroup,
                ) => true,
                (
...
fn expected_shape(expected: &ExpectedEvent) -> String {
    match expected {
        ...
        ExpectedEvent::Notification(n) => match n {
            ...
            ExpectedNotification::ProposalAcceptedGroup => {
                "Notif(ProposalAcceptedGroup)".to_string()
            }
...
pub fn game_accepted() -> ExpectedEvent {
    ExpectedEvent::Notification(ExpectedNotification::ProposalAcceptedGroup)
}
...
fn run_game_container_with_action_list_with_success_predicate(
    ...
    // side) yields exactly one ProposalAcceptedGroup membership or ProposalCancelled.
        ...
        for n in &lui.notifications {
            if let GameNotification::GameStatus { id, status, .. } = n {
                ...
                assert!(
                    ...
                    "player {i}: on-chain status for {id:?} but no ProposalAcceptedGroup member for that game. \
...
pub fn test_funs() -> Vec<(&'static str, &'static (dyn Fn() + Send + Sync))> {
    ...
    res.push(("test_proposal_accepts_custom_game_timeout", &|| {
        ...
        assert!(
            outcome.local_uis[0]
                .notifications
                .iter()
                .any(|n| matches!(n, GameNotification::ProposalAcceptedGroup { .. })),
    ...
    res.push(("test_proposal_accept_then_on_chain", &|| {
        ...
        assert!(
            p0_notifs
                .iter()
                .any(|n| matches!(n, GameNotification::ProposalAcceptedGroup { .. })),
            "Alice should see ProposalAcceptedGroup, got: {p0_notifs:?}"
        );
        ...
        assert!(
            p1_notifs
                .iter()
                .any(|n| matches!(n, GameNotification::ProposalAcceptedGroup { .. })),
            "Bob should see ProposalAcceptedGroup, got: {p1_notifs:?}"
        );
    }));
    ...
    res.push(("test_insufficient_balance_on_accept", &|| {
        ...
        // insufficient balance. Bob sees InsufficientBalance without a fake
        ...
        assert!(
            !p1_notifs
                .iter()
                .any(|n| proposal_accepted_contains(n, &GameID(3))),
            "Bob must not get ProposalAcceptedGroup for failed game 3, got: {p1_notifs:?}"
        );
        assert!(
            p1_notifs
                .iter()
    ...
    res.push(("test_atomic_local_batch_queue_and_finalization", &|| {
        let mut allocator = AllocEncoder::new();
        let mut rng = ChaCha8Rng::from_seed([41; 32]);
        let moves = [DebugGameTestMove::new(100, 0)];
        let mut sim_setup = setup_debug_test(&mut allocator, &mut rng, &moves).expect("setup");
        sim_setup
            .game_actions
            .push(SimScriptAction::AcceptSettlement(
                1,
        ...
        let mut holder = if outcome.cradles[0].has_potato_for_testing() {
            0
        } else {
            1
        };
        ...
        let make_proposal = |allocator: &mut AllocEncoder| GameProposal {
            sender_is_player_a: true,
            game_type: crate::session_phases::game_collection::game_type_for_package(
                allocator, "debug",
            ),
            timeout: Timeout::new(15),
            parameters: ProposalParameters::from_program_for_testing(
                allocator,
                sim_setup.args_program.as_ref(),
            )
            .expect("debug proposal parameters"),
        };

        let first_proposal = make_proposal(&mut allocator);
        ...
        let later_proposal = make_proposal(&mut allocator);
        outcome.cradles[holder]
            .propose(&mut allocator, &later_proposal)
            .expect("queue later valid proposal");

        let first = outcome.cradles[holder]
            .flush_and_collect(&mut allocator)
        ...
        assert_eq!(
            first
                .events
                .iter()
                .filter(|event| matches!(event, GameSessionEvent::OutboundMessage(_)))
                .count(),
            0,
            "failed planning must not emit a partial batch"
        );
        ...
        assert_eq!(
            ...
            2,
            "only the failing action should be removed"
        );
        ...
        outcome.cradles[holder ^ 1]
            .deliver_message(retry_message)
            .expect("deliver retry batch");
        outcome.cradles[holder ^ 1]
            .flush_and_collect(&mut allocator)
            .expect("receive retry batch");
        holder ^= 1;

        outcome.cradles[holder]
            .queue_game_action_for_testing(GameAction::QueuedCancelProposal(LocalProposalId(1)))
            .expect("queue finalization cancellation");
        outcome.cradles[holder]
            .fail_next_cached_unroll_update_for_testing()
            .expect("inject finalization failure");
        let failed_finalization = outcome.cradles[holder]
            .flush_and_collect(&mut allocator)
            .expect("collect finalization failure");
        assert!(failed_finalization.events.iter().any(|event| matches!(
            event,
            GameSessionEvent::Notification(GameNotification::ActionFailed {
                id: None,
                action: None,
        ...
        assert_eq!(
            outcome.cradles[holder].queued_game_action_count_for_testing(),
            1,
            "finalization failure must preserve the complete queue"
        );

        let finalized = outcome.cradles[holder]
            .flush_and_collect(&mut allocator)
            .expect("retry finalization");
        let finalized_message = finalized
            .events
            .iter()
            .find_map(|event| match event {
                GameSessionEvent::OutboundMessage(bytes) => Some(bytes),
            ...
            .expect("finalized batch");
        outcome.cradles[holder ^ 1]
            .deliver_message(finalized_message)
            .expect("deliver finalized batch");
        outcome.cradles[holder ^ 1]
            .flush_and_collect(&mut allocator)
            .expect("receive finalized batch");
        holder ^= 1;
        ...
        let post_cheat_proposal = make_proposal(&mut allocator);
        outcome.cradles[holder]
            .propose(&mut allocator, &post_cheat_proposal)
            .expect("queue action after deferred cheat");
        let deferred = outcome.cradles[holder]
            .flush_and_collect(&mut allocator)
        ...
        let deferred_message = deferred
            ...
            .expect("batch after deferred cheat");
        let deferred_batch =
            crate::session_phases::peer_wire::decode_peer_message(deferred_message)
                .expect("decode");
        assert!(matches!(
            deferred_batch,
            PeerMessage::Batch { ref actions, .. }
        ...
        assert_eq!(
            outcome.cradles[holder].queued_game_action_count_for_testing(),
            1,
            "deferred cheat must remain queued"
        );
        outcome.cradles[holder ^ 1]
            .deliver_message(deferred_message)
            .expect("deliver deferred-cheat batch");
        outcome.cradles[holder ^ 1]
            .flush_and_collect(&mut allocator)
            .expect("receive deferred-cheat batch");
        holder ^= 1;

        outcome.cradles[holder]
            .queue_game_action_for_testing(GameAction::QueuedCancelProposal(LocalProposalId(2)))
            .expect("queue action before shutdown");
        outcome.cradles[holder]
            .queue_game_action_for_testing(GameAction::CleanShutdown)
            .expect("queue deferred shutdown");
        outcome.cradles[holder]
            .queue_game_action_for_testing(GameAction::AcceptSettlement(GameID(777)))
            .expect("queue action after shutdown");
        let shutdown_deferred = outcome.cradles[holder]
            .flush_and_collect(&mut allocator)
            .expect("flush action before shutdown");
        let messages = shutdown_deferred
            .events
            .iter()
            .filter_map(|event| match event {
                GameSessionEvent::OutboundMessage(bytes) => Some(
                    crate::session_phases::peer_wire::decode_peer_message(bytes).expect("decode"),
                ),
                _ => None,
            })
            .collect::<Vec<_>>();
        assert!(matches!(
            messages.as_slice(),
            [
                PeerMessage::Batch { actions, .. },
                PeerMessage::RequestPotato(())
            ] if actions.len() == 1 && matches!(actions[0], BatchAction::CancelProposal(_))
        ));
        assert!(!shutdown_deferred.events.iter().any(|event| matches!(
            event,
            GameSessionEvent::Notification(GameNotification::ActionFailed { .. })
        )));
        assert_eq!(
            outcome.cradles[holder].queued_game_action_count_for_testing(),
            2,
            "shutdown and later actions must retain their original order"
        );
    }));
