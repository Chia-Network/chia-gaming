                ...
                if let Some(redo_data) = &redo {
                    ...
                    if is_my_turn {
                        ...
                        let conditions = CoinCondition::from_puzzle_and_solution(
                            env.allocator,
                            &transaction.bundle.puzzle.to_program(),
                            &transaction.bundle.solution.p(),
                        )?;
                        debug!("conditions returned {conditions:?}");
                        let new_ph = if let Some(p) = conditions
                            .iter()
                            .filter_map(|c| {
