...
use crate::common::types::{Aggsig, AllocEncoder, CoinString, Error, Node, Program, ProgramRef, Puzzle};
...
impl Spend {
    pub fn from_clvm(allocator: &mut AllocEncoder, data: NodePtr) -> Result<Spend, Error> {
        let lst =
            if let Some(lst) = proper_list(allocator.allocator(), data, true) {
                lst
            } else {
        ...
        let signature_atom =
            if let Some(s) = atom_from_clvm(allocator, lst[2]) {
                s
            } else {
        ...
        Ok(Spend { puzzle, solution: solution.into(), signature })
    }
}
...
impl CoinSpend {
    pub fn from_clvm(allocator: &mut AllocEncoder, data: NodePtr) -> Result<CoinSpend, Error> {
        ...
        let coin_bytes =
            if let Some(by) = atom_from_clvm(allocator, lst[0]) {
                by
            } else {
