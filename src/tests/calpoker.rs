...
#[cfg(feature = "sim-tests")]
use crate::games::calpoker::{CalpokerHandValue, RawCalpokerHandValue, get_final_cards_in_canonical_order, select_cards_using_bits};
#[cfg(feature = "sim-tests")]
...
pub fn test_moves_1(allocator: &mut AllocEncoder) -> (u8, u8, [GameAction; 5]) {
...
fn test_play_calpoker_happy_path() {
    let mut allocator = AllocEncoder::new();
    let (ad, bd, moves) = test_moves_1(&mut allocator);
...
fn test_verify_endgame_data() {
        ...
        game_action_results[game_action_results.len() - 1]
    {
        ...
        let with_message: Vec<ReadableMove> = game_action_results.iter().filter_map(|m| {
        ...
        let (alice_initial_cards, bob_initial_cards) = decode_readable_card_choices(
            &mut allocator,
            with_message[0].clone()
        ...
        let (alice_final_hand, _) = select_cards_using_bits(&alice_final_cards, 185);
        let (bob_final_hand, _) = select_cards_using_bits(&bob_final_cards, 205);
...
fn test_verify_bob_message() {
    ...
    let mut allocator = AllocEncoder::new();
    let micro_test_hex = read_hex_puzzle(&mut allocator, "clsp/test/test_handcalc_micro.hex").expect("should exist");
    let (ad, bd, moves) = test_moves_1(&mut allocator);
    ...
    todo!();
    // make_cards(&alice_message_bytes, entropy.bytes(), Amount::new(200));
...
fn test_play_calpoker_end_game_reward() {
    ...
    let (ad, bd, moves_slice) = test_moves_1(&mut allocator);
    let mut moves = moves_slice.to_vec();
