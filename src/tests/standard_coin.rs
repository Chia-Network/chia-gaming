...
// The key above hashes to 0x6951..., leading bit clear, where reading the hash signed and
// unsigned agree. This one hashes to 0xcef0..., leading bit set, where they do not.
#[test]
fn test_calculate_synthetic_offset_high_bit() {
    let pk_bytes: [u8; 48] = [
        0xa5, 0x6f, 0xf6, 0x5f, 0xd9, 0xe3, 0x47, 0x84, 0x31, 0xbc, 0x70, 0xe6, 0x85, 0xdf, 0xdf,
        0x09, 0xf6, 0xb4, 0xdc, 0x0a, 0x44, 0x29, 0x62, 0xee, 0x97, 0x99, 0xca, 0x09, 0xad, 0x2a,
        0x53, 0xac, 0xf9, 0xbe, 0xf2, 0xe9, 0xab, 0x9f, 0x20, 0x3f, 0x32, 0xc0, 0x16, 0x36, 0xde,
        0xe1, 0x34, 0xa5,
    ];
    let pk = PublicKey::from_bytes(pk_bytes).expect("should be ok");
    let default_hidden_puzzle_hash = PuzzleHash::from_bytes(DEFAULT_HIDDEN_PUZZLE_HASH);
    let offset = calculate_synthetic_offset(&pk, &default_hidden_puzzle_hash);
    let want_offset_bytes = [
        0x42, 0xde, 0x7e, 0xed, 0x51, 0x2f, 0xbd, 0x4a, 0x9c, 0x81, 0x91, 0x5a, 0xef, 0xaa, 0xa4,
        0x91, 0x65, 0x57, 0x50, 0xaa, 0x2b, 0x08, 0xf7, 0x58, 0xe0, 0xc6, 0x75, 0xd5, 0x7b, 0x15,
        0xdf, 0xc3,
    ];
    let want_offset = number_from_u8(&want_offset_bytes);
    assert_eq!(offset, want_offset);
