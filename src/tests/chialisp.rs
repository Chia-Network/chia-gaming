...
fn test_make_cards() {
    ...
    let converted_source =
        convert_from_clvm_rs(allocator.allocator(), loc.clone(), source_data).expect("should work");
    let result = run(
        allocator.allocator(),
...
fn test_mergein() {
    ...
    for t in tests.iter() {
        ...
        .expect("should run")
        .1;
        assert_eq!(disassemble(allocator.allocator(), result, None), t.1);
    }
}
...
fn test_find_straight_high() {
    ...
    .expect("should run")
    .1;
    assert_eq!(disassemble(allocator.allocator(), result, None), "5",);
}
