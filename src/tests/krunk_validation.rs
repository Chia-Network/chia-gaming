...
const VALIDATOR_NAMES: [&str; 3] = ["commit", "guess", "clue"];
...
fn test_krunk_commit_happy() {
    ...
    let commit = read_hex_puzzle(allocator, "clsp/games/krunk/onchain/commit.hex").unwrap();
...
fn test_krunk_guess_slash_invalid_word() {
    let mut allocator = AllocEncoder::new();
    let guess = read_hex_puzzle(allocator, "clsp/games/krunk/onchain/guess.hex").unwrap();
    let dictionary = words_to_list(&mut allocator, &["crane", "slate"]);
    let state = {
        let a = allocator.allocator();
        let commit = a.new_atom(&[0xCD; 32]).unwrap();
        let tail = a.new_pair(commit, NodePtr::NIL).unwrap();
        let tail = a.new_pair(NodePtr::NIL, tail).unwrap();
        a.new_pair(NodePtr::NIL, tail).unwrap()
    };
    let (code, _) = run_validator_step(
