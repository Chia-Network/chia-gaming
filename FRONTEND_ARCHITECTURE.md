...
All game messages between peers are relayed through the tracker's socket.io
server. Both players connect to the same tracker with a shared token, and the
tracker routes messages between them. This is simple and works well behind NATs,
...
2. A **socket.io connection** to the tracker's relay endpoint. This carries all
   real data: match notifications and game messages. The protocol is defined in
...
the iframe and socket internally by matching the token, so it knows which visual
lobby session corresponds to which socket.
...
The tracker has two kinds of socket.io connections per player:

1. **Lobby socket** — used by the lobby iframe for presence and matchmaking.
2. **Game socket** — used by the player app's `TrackerConnection` for game
   message relay.

Both sockets connect to the same socket.io server. The tracker links them by
`session_id` (lobby iframe sends `join` with `session_id`, game socket sends
`identify` with `session_id`, and the tracker maps session → player internally).
If the game socket's `identify` arrives before the lobby iframe's `join`, the
tracker buffers it and completes registration when the join arrives.
...
| `challenge` | `{ target_id, game, amount, per_game }` | Challenge another player to a game |
| `challenge_accept` | `{ challenge_id }` | Accept a pending challenge |
...
linked by a random token) and emits `matched` to both players' game sockets.
...
| Event | Payload | Purpose |
|-------|---------|---------|
| `identify` | `{ session_id }` | Sent immediately after the socket opens. Links this socket to the player's lobby session. |
| `message` | `{ data }` | Send opaque bytes to the paired peer. The tracker relays verbatim. |
...
| `connection_status` | `{ has_pairing, token?, game_type?, amount?, per_game?, i_am_initiator?, peer_connected? }` | Definitive status report sent on every `identify`. The player app uses this to reconcile tracker state against its local save (see [Reconnect Reconciliation](#reconnect-reconciliation)). |
| `peer_reconnected` | `{}` | The other player's game socket just re-registered. Tells this side to re-send un-acked messages. |
...
3. The tracker associates this socket with the lobby session. If a previous
   socket exists for this player, the tracker kills the old one and replaces it.
...
5. If a pairing exists and the peer is connected, the tracker also sends
   `peer_reconnected` to the peer's game socket.
6. When a challenge is accepted in the lobby, the tracker sends `matched` to
   both players' game sockets. The player app starts the WASM cradle and game
   session.
...
**What the tracker holds per paired session:** Just the two sockets and the
pairing metadata (player IDs, token, game parameters). No message log, no
delivery receipts, no game state. When a socket drops, messages to that peer are
silently discarded. When a new socket claims the slot (step 3), the tracker
resumes routing. Lobby socket disconnects do NOT remove the player from the
lobby — the 10-minute sweep in `lobbyState.ts` handles cleanup of truly departed
players. The lobby iframe re-`join`s on reload, refreshing `lastActive`.
...
application-level pings on top of Socket.io's transport-level heartbeat.

**Events:** `tracker_ping` and `tracker_pong` — direct socket.io events between
client and server (not relayed to peers, distinct from the peer `{ ping }`
messages that travel inside the `message` channel).

**Server side (`index.ts`):**

- Maintains a `lastHeardFrom` map keyed by `socket.id`, updated on every inbound
  event (including `tracker_pong`).
- The existing 15-second sweep interval sends `tracker_ping` to all connected
  sockets. Any socket whose `lastHeardFrom` exceeds 60 seconds is disconnected,
  triggering the standard disconnect cleanup handler.
- Responds to `tracker_ping` from clients with `tracker_pong`.

**Game socket client (`TrackerConnection`):**

- Sends `tracker_ping` every 15 seconds. Responds to `tracker_ping` from the
  server with `tracker_pong`.
- Tracks `lastTrackerHeardFrom`, updated on every inbound event.
- If 60 seconds pass with no activity, force-disconnects the socket. Socket.io
  auto-reconnection (see below) takes over.
- Timer is stopped on disconnect and restarted on reconnect.

**Lobby socket client (`useLobbySocket`):**

- Same protocol: sends `tracker_ping` every 15 seconds, responds to server
  pings, tracks `lastTrackerHeardFrom`, disconnects on 60-second timeout.
- On reconnect, re-emits `join` to re-register the lobby socket.

#### Tracker Reconnection

Both socket types use Socket.io's built-in reconnection with randomized
exponential backoff:
...
The existing `socket.on('connect')` handler re-sends `identify` (game socket) or
`join` (lobby socket) on every connect, including reconnects. For game sockets,
this triggers a `connection_status` response from the tracker, which feeds into
...
callbacks for logging/diagnostics.
...
tracker — both on initial page load and on mid-session socket reconnects.
...
When Socket.io reconnects during an active session, `connection_status` fires
...
- **Tracker connection** — fetches the tracker URL, creates the
  `TrackerConnection` socket, and sets up the lobby iframe
...
- **Session token** — passed via URL parameter so the tracker can link the
  iframe to the game socket

The lobby iframe uses the socket.io challenge protocol (see above) to trigger
