...
2. **Tracker** — A separate dynamic service that provides two things: a lobby UI
   ...
   relay that ferries game messages between peers via REST endpoints and SSE streams.
   The tracker is
...
2. A **game relay channel** to the tracker (`/game/events` plus `/game/*` POST
   endpoints). This carries match notifications and game messages. The protocol
   is defined in
...
visual lobby session corresponds to which game stream.
...
1. **Lobby channel** — used by the lobby iframe for presence and matchmaking
   (`/lobby/events` + `/lobby/*` endpoints).
2. **Game channel** — used by the player app's `TrackerConnection` for game
   message relay.

Both channels connect to the same tracker service. The tracker links them by
`session_id` (lobby iframe sends `join` with `session_id`, game channel sends
`identify` with `session_id`, and the tracker maps session → player internally).
If the game channel `identify` arrives before the lobby iframe's `join`, the
...
1. Player opens an SSE stream to the tracker game channel.
...
3. The tracker associates this game channel with the lobby session. If a
   previous game channel exists for this player, the tracker closes the old
   stream and replaces it.
...
delivery receipts, no game state. When a stream drops, messages to that peer are
...
**Events:** peer `ping` payloads travel inside relayed `message` events. This is
distinct from SSE keepalive comments between each client and the tracker.

**Server side (`index.ts`):**

- Maintains SSE clients for lobby and game channels, each with a small replay
  ring buffer for reconnect continuity.
- Runs a 15-second sweep for lobby/game housekeeping and writes SSE keepalive
  comments to active streams.

**Game channel client (`TrackerConnection`):**

- Uses `EventSource` for `/game/events` and re-sends `identify` when the stream
  reconnects.
- Sends periodic peer `{ "ping": true }` payloads through `/game/send`.

**Lobby channel client (`useLobbySocket`):**

- Uses `EventSource` for `/lobby/events`.
- On reconnect, re-emits `join` to re-register the lobby channel.

#### Tracker Reconnection

Both channels use browser-managed `EventSource` reconnect behavior. On
...
The lobby iframe uses the REST/SSE challenge protocol (see above) to trigger
...
| `front-end/src/services/TrackerConnection.ts` | Game relay client (`/game/events` + `/game/*` POST) |
| `front-end/src/types/ChiaGaming.ts` | TypeScript types for WASM interface and game data |
| `lobby/lobby-frontend/src/useLobbySocket.ts` | Lobby channel hook (`useLobbySocket`): `/lobby/events` + `/lobby/*` requests |
